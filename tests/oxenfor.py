...
BASE_URL = "https://<some-hostname-here>"
                ...
                result = page.evaluate("""
                    () => {
                        ...
                        if (citation) {
                            ...
                            return {success: true, value: citation.value};
                        }
                        return {success: false, error: "Citation field not found"};
                    }
                """)
                print(f"Citation result: {result}")
                
                # Add a wait after citation to ensure it completes
                page.wait_for_timeout(1000)
                print("Waited after citation JS")
                
                # Take screenshot to see the state after citation
                page.screenshot(path="after_citation.png")
                print("Screenshot taken after citation")
                
            except Exception as e:
                print(f"Error with citation JS: {e}")
                print(traceback.format_exc())
                page.screenshot(path="citation_error.png")
                print("Error screenshot saved")
            
            # After decoration citation and before the Save button
            print("Preparing to handle imprisonment section...")
            
            # Click Prisoner of War Details accordion to expand it
            print("About to click Prisoner of War Details...")
            page.click("button[data-bs-target='#prisoner-of-war-details-none-recorded']")
            print("Clicked Prisoner of War Details")
            
            # Wait for accordion to expand
            page.wait_for_timeout(1000)
            
            # Select POW camp first
            print("Selecting POW camp...")
            page.select_option("select[name='imprisonment-0-pow_camp']", "1")
            print("Selected POW camp")
            
            # Fill dates
            print("Filling dates...")
            page.fill("input[name='imprisonment-0-date_from']", "1940-06-01")
            page.fill("input[name='imprisonment-0-date_to']", "1945-05-08")
            print("Filled imprisonment dates")
            
            # Fill notes using JavaScript
            print("Setting imprisonment notes...")
            page.evaluate("""
                () => {
                    const notes = document.querySelector('textarea[name="imprisonment-0-notes"]');
                    if (notes) {
                        notes.focus();
                        notes.value = "Test imprisonment notes";
